...
ETOTAL_RE = re.compile(r"^\s*>+\s*Etotal=\s*(\S+)", re.MULTILINE)


class AbinitAboParser(BaseOutputFileParser):
    ...
    def parse(content: str) -> dict[str, Any]:
        ...
        etotal_match = ETOTAL_RE.search(content)
        if etotal_match is not None:
            result["total_energy"] = float(etotal_match.group(1))
